...
強制パスワード再認証によるストレージごとのキープアライブメカニズム。

- **仕組み**：
  1. 管理ページからストレージごとにログイン間隔（分）を設定。
  2. 各間隔で、システムは古いドライバインスタンスを破棄し、キャッシュされたトークンをすべて除去。
  3. 新しいドライバインスタンスを作成 — キャッシュトークンが見つからず、ユーザー名/パスワードログインを実行。
  4. パスワードログインが失敗した場合、バックアップトークンを使用したトークンリフレッシュに自動フォールバック。

- **有効化方法**：
  1. `http://<host>:5244/@manage/login-schedule` を開く（管理サイドバーの「タスク」下からもアクセス可能）。
  2. 各ストレージのログイン間隔（分）を設定。`0` = 無効。
  3. 保存をクリック。
...
| `/@manage/login-schedule` | GET | ストレージログインスケジュール管理ページ | クライアントトークン |
